package Test01_Database.T5_Hoewon;
...
public class HoewonService {
	...
	public void setHoewonInput() {
		while(true) {
			...
			if(vo != null) {
				System.out.println("같은 이름이 존재합니다. 다른 이름으로 가입해주세요.\n");
				continue;
			}
		...
		while(true) {
			try {
				...
				if(select == 1) {
					...
					if(vo2 != null) {
						System.out.println("같은 이름이 존재하거나 이름의 형식이 틀렸습니다.\n");
						continue;
					}
					else break;
				}
			} catch (InputMismatchException e) {
				System.out.println("잘못입력하셨습니다. 수정하려는 항목을 확인해주세요.\n");
				scanner.next();
				continue;
			}
		}
